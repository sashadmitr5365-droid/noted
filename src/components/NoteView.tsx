        ...
        >
          ...
          <GlassCard radius={settings.cardRadius} style={settings.glassStyle}>
            ...
            <div className="relative px-3.5 py-3.5">
              ...
              {hasBody && (
                <div
                  className="doc-render t-body mt-3 break-words"
                  style={{
...
function GlassCard({
  ...
  if (style === "clear") {
    ...
    return (
      ...
      >
        {/* Blurred copy of the page background — guarantees the blur
            is visible on every browser, even where backdrop-filter
            fails. The card sits on top of this with a translucent
            background. */}
        <div
          aria-hidden
          style={{
            ...
            inset: 0,
            borderRadius: `${radius}px`,
            background: composeBackground(bg),
            filter: "blur(12px)",
        ...
        <article
          ...
          style={{
            ...
            zIndex: 1,
            borderRadius: `${radius}px`,
            // Glass card per spec — no saturate, no contrast (those
            // make the card appear brighter than the backdrop). Just
            // a moderate blur so the underlying stripes/gradients are
            // visibly softened but still readable through the card.
            background: "rgba(255, 255, 255, 0.05)",
