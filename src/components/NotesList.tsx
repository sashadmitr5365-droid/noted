...
export default function NotesList({ initialNotes }: { initialNotes: NoteListItem[] }) {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <header className="sticky top-0 z-30 -mt-px border-b border-white/[0.04] px-5 pt-7 pb-4">
      ...
      {stats.total > 0 && (
        <Link
          ...
          className="fixed bottom-6 right-1/2 z-40 flex h-14 w-14 translate-x-[200px] items-center justify-center rounded-full bg-white text-ink-950 shadow-[0_18px_40px_-8px_rgba(255,255,255,0.4)] transition-transform active:scale-95 hover:scale-105"
        >
...
function EmptyState() {
  return (
    <div className="mt-10 flex flex-col items-center text-center">
      ...
      <Link
        href="/new"
        className="mt-6 inline-flex h-11 items-center gap-2 rounded-full bg-white px-5 text-sm font-semibold text-ink-950 shadow-[0_10px_28px_-8px_rgba(255,255,255,0.3)] transition-transform active:scale-95 hover:bg-white/90"
      >
        <IconPlus className="h-4 w-4" />
        Создать заметку
      </Link>
