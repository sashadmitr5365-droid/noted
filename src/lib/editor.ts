...
export function plainTextToDoc(text: string): Doc {
  ...
  // newline (line break). Each paragraph becomes a SINGLE <p> block
  // with multiple segments separated by <br> at render time — that way
  // one Enter shows as a normal line break (like in the editor), and
  // two Enters create a new paragraph (visual gap via CSS margin).
  ...
  for (const para of paragraphs) {
    ...
    if (lines.length === 1) {
      blocks.push({
        type: "p" as const,
        segments: lines[0] ? [{ text: lines[0] }] : [],
      });
    } else {
      blocks.push({
        type: "p" as const,
        segments: lines.filter((l) => l !== undefined).map((l) => ({
          text: l,
        })),
      });
    }
...
function renderSegments(segments: InlineSegment[]): string {
  // Inline segments within a single block are separated by <br> at
  // render time so that a single newline in the editor becomes a
  // visual line break, while a double newline becomes a new block
  // (rendered as a separate <p> with CSS margin between them).
  return segments
    .map((s, i) => {
